...
function llenarSelectSpe(items){

    for (let j = 0; j < items.length; j++) {
        console.log(items[j])
        $('#doctor').append(`<option value="${items[j].id}">${items[j].name}</option>`);
        $('#doctor').val("");
    }
...
function tableRespuestaSpe(items) {
    console.log(items[0].doctors[0]["name"]);
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        myTableSpe += `<td>${items[0].doctors[0]["name"]}</td>`;
